  ...
  const createMonthlyDues = (e) => {
    ...
    list.map(function(c) {
        for(var i = 0; i <= list.length; i++){
            ...
            setDuesList({m_d:v,id:c.id,nm:c.name,bc: c.batch,lg:c.lga})
        }
        return rr;
    });
    // for(var l = 0; l <= list.length; l++){
    //     list.map(function(c) {
    //         for(var i = 0; i <= duesKeys.length; i++){
    //             const d = `dues_${i}`;
    //             const k = duesKeys[i];
    //             const v = duesValues[i];
    //             k === `dues_${c.id}` ? rr.push({m_d:v,id:c.id,nm:c.name,bc: c.batch,lg:c.lga}) : console.log('doesnt match') ;
    //         }
    //         return rr;
    //     });
    // }
