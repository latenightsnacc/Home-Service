    ...
    const createMonthlyDues = (e) => {
        ...
        duesKeys.forEach(function (f) {
            ...
            list.map(function(c) {
                let l = c.id;
                if(f.endsWith(l)){
                    rr.push({dues_for: date.month, year:date.year,collection: newCollection, dues:returnValue(duesValues, count),cds_group:c.cds_group,id:c.id,name:c.name,statecode:c.state_code,batch: c.batch,lga:c.lga}); 
                } else {
                console.log('end');  
                }
                count++;
                                    ...
                                    <tr className='text-xs hover:bg-gray-50 hover:text-green-500' id={val.id} key={val.id}>
                                        ...
                                        <td className="text-left">
                                            <input 
                                                ...
                                                id={"amount"}
                                                value={`₦${amount}`} 
                                                className="form-input mt-2 hover:text-green-400 focus:text-green-400 focus:no-outline checked:text-green-400 checked:border-0 checked:border-yellow-400 "
                                                onChange={getAmount}
                                                />
                                        </td>
