...
const NewMonthlyDues = () => {
    ...
    const getDate =(e) => {
        let a = e.t
        setDate({
            date_recorded:(e.target.value).toLocaleDateString(),
            month: months[(e.target.value).getMonth()],
    ...
    return(
        ...
        <Layout>
            ...
            <Container>
                <form onSubmit={trySending} className="px-1">
                    ...
                    <table className="table table-sm table-bordered border-success table-hover text-xs md:text-sm bg-white rounded">
                        <thead>
                            ...
                            {list.map(val => {
                            return (
                                <tr className='text-xs md:text-sm font-medium hover:bg-gray-50 hover:text-green-500' id={val.id} key={val.id}>
