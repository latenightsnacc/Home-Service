...
const NewMonthlyDues = () => {
    ...
    const paidDues = (e) => {
        let t;
        const tr = document.getElementsByTagName('tr')[0].innerText;
        for(let i = 0; i <= list.length; ++i) {
            if (list[i].id === 1 && e.target.checked === true) {
                t = document.getElementsByTagName('td')[0].innerText;
            }
                ...
                <form onSubmit={createMonthlyDues} class="er" id="form">
                    ...
                    <div className="container">
                        <table className="table table-sm table-auto table-bordered border-success table-hover text-xs md:text-sm bg-white rounded">
                                ...
                                {list.map(val => {
                                return (
                                    <tr className='text-xs hover:bg-gray-50 hover:text-green-500' id={val.id} key={val.id} onClick={getDetails}>
