...
const NewMonthlyDues = () => {
  ...
  const createMonthlyDues = (e) => {
    ...
    const rr = [];
    // duesKeys.map( k => duesValues.map(v => list.map(c => k === `dues_${c.id}` ? setDuesList({...duesList, corper_id: c.id, corper_name: c.name, corper_statecode: c.state_code, corper_batch: c.batch, corper_lga: c.lga, monthly_due: v}) : console.log('nothing'))));
    console.log(`dues_${list[0].id}`);
    for(var l = 0; l <= list.length; i++) {
        for(var i = 1; i <= duesKeys.length; i)
    }
        for(var i = 1; i <= list.length; i++){
            const d = `dues_${i}`;
            // console.log(d);
            const k = duesKeys[i];
            d === k ? console.log(list[i].id) : console.log("don't match");
            if(duesKeys[i] === d){
                console.log(duesKeys[i]);
                // rr.push({
                //     monthly_due: duesValues[i],
                //     corper_id: list[i].id,
                //     corper_name: list[i].name,
                //     corper_batch: list[i].batch,
                //     corper_lga: list[i].lga
                // })
            }
        }
