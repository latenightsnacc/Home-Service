    ...
    const createMonthlyDues = (e) => {
        ...
        duesKeys.forEach(function (f) {
            ...
            list.map(function(c) {
                let l = c.id;
                if(f.endsWith(l)){
                    rr.push({month: date.month, year:date.year,collection: newCollection, dues:returnValue(duesValues, count),id:c.id,name:c.name,statecode:c.state_code,batch: c.batch,lga:c.lga}); 
                } else {
                console.log('end');  
                }
                count++;
        ...
        try {
            Axios.post("http://localhost:3001/newmonthlydues", {
                title: `${newCollection}_${date.month}_$`,
                ...rr
            }).then( (res) => {
                console.log(res);
            })
                ...
                <form onSubmit={createMonthlyDues} class="er" id="form">
                    <div className="container text-gray-800">
                        <div className="flex flex-row items-center justify-between">
                            ...
                            <div>
                                    <label className="mr-2" for={"month"}>Month: </label>
                                    <input
                                    type={"month"}
                                    id={"month"}
                                    name={"month"} 
