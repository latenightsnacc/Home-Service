            ...
            for(let a in amountKeys) {
                ...
                list.map(function(c) {
                    ...
                    if(f.endsWith(l) && a.endsWith(l)){
                        rr.push({dues_for: date.month, year:date.year,collection: newCollection, newCollection_dues:returnValue(duesValues, count, 'obj'),amt_paid:returnValue(amountValues, count),cds_group:c.cds_group,id:c.id,name:c.name,statecode:c.state_code,batch: c.batch,lga:c.lga}); 
                    } else {
                    return 'end';  
                    }
                    count++;
