...
const NewMonthlyDues = () => {
  ...
  const createMonthlyDues = (e) => {
    ...
    duesKeys.map(k => duesValues.map(v => list.map(c => k === `dues_${c.id}` ? setDuesList({...duesList, corper_id: c.id, corper_name: c.name, corper_statecode: c.state_code, corper_batch: c.batch, corper_lga: c.lga,})) ) );
    duesValues.map((k) => {
        return k;
    })
