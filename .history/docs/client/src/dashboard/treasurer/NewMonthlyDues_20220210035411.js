  ...
  const createMonthlyDues = (e) => {
    ...
    duesKeys.forEach(function (f) {
        ...
        list.map(function(c) {
            let l = c.id;
            if(f.endsWith(l)){
                 rr.push({month: month.month, monthly_dues:returnValue(duesValues, count),id:c.id,name:c.name,statecode:c.,batch: c.batch,lga:c.lga}); 
            } else {
              console.log('end');  
            }
            count++;
