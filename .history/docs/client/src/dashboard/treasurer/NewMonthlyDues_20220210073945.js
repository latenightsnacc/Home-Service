    ...
    const createMonthlyDues = (e) => {
        ...
        duesKeys.forEach(function (f) {
            ...
            list.map(function(c) {
                let l = c.id;
                if(f.endsWith(l)){
                    rr.push({dues_for: date.month, year:date.year,collection: newCollection, dues:returnValue(duesValues, count),amount_paid: amount`₦${amount}`,cds_group:c.cds_group,id:c.id,name:c.name,statecode:c.state_code,batch: c.batch,lga:c.lga}); 
                } else {
                console.log('end');  
                }
                count++;
