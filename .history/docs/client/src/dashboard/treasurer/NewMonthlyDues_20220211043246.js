    ...
    const createMonthlyDues = (e) => {
        ...
        duesKeys.forEach(function (f) {
            list.map(function(c) {
                ...
                if(f.endsWith(l)){
                    rr.push({
                        dues_for: date.month, year:date.year,collection: newCollection, newCollection_dues:returnValue(duesValues, count),amt_paid:returnValue(amountCollected, count, true),cds_group:c.cds_group,id:c.id,name:c.name,statecode:c.state_code,batch: c.batch,lga:c.lga}); 
                    console.log(count);
                } else {
                return 'end';  
                }
