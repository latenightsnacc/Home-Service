    ...
    if (error) return 'error';
      ...
      const recordAttendance = (e) => {
        ...
        for(var i = 0;i < s.length;i++){
            const d = document.getElementsByTagName('input')[i];
            let a = {};
            if(i === 0){
                for(var j = i - 1;j < 8;j++){
                    const d = document.getElementsByTagName('input')[j];
                    a[n[j]] = d.value;
                }
                rr.push(a);
            }else if(i % 8 === 0){
                for(var k = i;k <= i + 8;k++){
                    const d = document.getElementsByTagName('input')[k];
                    a[n[k]] = d.value;
                }
                rr.push(a);
            }
        ...
        // attendance.map(a => {
        //     console.log(a);
            // newAttendance.id.attendance = a["attendance_"+i];
            // i++;
            // return a;
        // })
