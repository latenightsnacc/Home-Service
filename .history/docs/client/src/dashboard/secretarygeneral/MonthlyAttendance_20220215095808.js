...
const MonthlyAttendance = () => {
    ...
    for(const row in results){
        const n = results[row].attendance_date;
        results[row].attendance_date === n ? dates.date_1.push(results[row]) : dates.date_2.push(results[row])
    }
    
   
    const mValues = Object.values(results);
                ...
                {mValues.map((mon,key) => {
                   if(mon.length > 0){
                       ...
                       return (
                        ...
                        onClick={() => {navigate(`../dashboard/secretarygeneral/attendance/${mon[0].attendance_date}/${mon[0].attendance_month}`)}}>
                            <AttendanceSummary 
