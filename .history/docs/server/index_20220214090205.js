...
app.post("/monthlyDues", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            db.query("SELECT * FROM monthly_dues WHERE date_recorded = ? AND payment_status = ? AND amount_paid = ? AND corper_id = ?",[collection_date,payment_status,a,c_id], (err, result) => {
                if(err) {
                    console.log(err)
