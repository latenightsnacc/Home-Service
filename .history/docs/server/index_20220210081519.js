    ...
    console.log(req.body);
        try {
            ...
            for(let a in req.body) {
                ...
                corper_lga = a.lga;
                cds = a.cds_group
                db.query('INSERT INTO dues_collection (date, venue, topic, start_time, end_time, type, minutes) VALUES(?,?,?,?,?,?,?)', [date,venue,topic,start,end,type,minutes], (err, result) => {
    //          if(err){
    //              console.log(err)
    //          } else {
    //              res.send("Values Inserted");
