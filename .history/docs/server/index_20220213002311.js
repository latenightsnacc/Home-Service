...
app.post("/eventDues", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            let c_lga = req.body[i].lga;
            let c_cds = req.body[i].cds_group;
            let project_title = `${collection_for} for ${pro}`;
            console.log([pro,project_title,payment_status,a_paid,c_id,c_name,c_statecode,c_batch,c_lga,c_cds]);
            try{
                db.query('INSERT INTO project_dues (pro,project_title,payment_status,amount_paid,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,corper_cds) VALUES(?,?,?,?,?,?,?,?,?,?)',
                    [pro,project_title,payment_status,a_paid,c_id,c_name,c_statecode,c_batch,c_lga,c_cds], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
                        console.log(`Corper:${c_name} Dues:${collection_for} Project Title:${project_title} C.D.S:${c_cds}  Status: ${payment_status} recorded.`);
