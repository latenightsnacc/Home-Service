...
app.post("/try", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            try{
                db.query('INSERT INTO dues_collection (collection_date,collection_month,collection_year,amount_paid,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,cds_group) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
                    [collection_date,collection_month,collection_year,collection_for,c_id,c_name,c_statecode,c_batch,c_lga,c_cds,c_attendance,comment], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
