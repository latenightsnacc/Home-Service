...
app.post("/try", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            let collection_for = req.body[i].newCollection;
            let payment_status = req.body[i].newCollection;
            ...
            console.log([collection_date,collection_month,collection_year,collection_for,a_paid,c_id,c_name,c_statecode,c_batch,c_lga,c_cds]);
            try{
                db.query('INSERT INTO dues_collection (collection_date,collection_month,collection_year,collection_for,amount_paid,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,cds_group) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
                    [collection_date,collection_month,collection_year,collection_for,a_paid,c_id,c_name,c_statecode,c_batch,c_lga,c_cds], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
                        
                        console.log(`Corper:${c_name} Dues:${} in C.D.S:${c_cds}  on ${a_date} has been recorded.`);
                    }
                })
            }catch(e){
                console.log(e)
