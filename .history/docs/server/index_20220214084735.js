...
app.post("/eventDues", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            let c_lga = req.body[i].lga;
            let c_cds = req.body[i].cds_group;
            db.query("SELECT * FROM event_dues WHERE attendance_date = ? AND attendance_type = ? AND attendance_status = ? AND corper_id = ?",[date,attd_type,attd_status,c_id], (err, result) => {
                if(err) {
                    console.log(err)
