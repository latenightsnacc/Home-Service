...
app.post("/trynew", (req,res) => {
    ...
    console.log(lt);
    try{
        for(var i = 0; i <= Object.keys(req.body); i++){
            ...
            try{
                db.query('INSERT INTO attendance (attendance_date,attendance_month,attendance_year,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,corper_cds,attedance_type,corper_attendance,comment) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
                    [a_date,a_month,a_year,c_id,c_name,c_statecode,c_batch,c_lga,c_cds,a_type,c_attendance,comment], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
                        res.send(`Attendance for ${a_type} on ${a_date} has been saved.`);
                        console.log(`Attendance for ${a_type} on ${a_date} has been saved.`);
                    }
                })
            }catch(e){
                console.log(e)
