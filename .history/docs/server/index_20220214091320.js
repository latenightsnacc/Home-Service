    ...
    try {
        ...
        db.query("SELECT * FROM  notes WHERE date = ? AND type = ? AND minutes = ? ",[date,type,minutes], (err, result) => {
            if(err) {
                ...
                db.query('REPLACE INTO notes (date,month,year, venue, topic, start_time, end_time, type, minutes) VALUES(?,?,?,?,?,?,?,?,?)', [date,month,year,venue,topic,start,end,type,minutes], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
