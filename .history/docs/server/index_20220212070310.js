...
app.get("/dues", (req,res) => {
    for(let i=0; i < months.length; i++){
        let month = months[i];
        db.query(`SELECT * FROM dues_collection WHERE collection_for = 'monthly_dues' AND collection_month = '${month}' `, (err,result) => {
            if(err) {
                console.log(err)
            } else {
                res.send(result);
...
app.get("/dues/:collectionMonth/:collectionYear/:collectionFor", (req,res) => {
    const collectionM = req.params.duesId;
    const collectionY = req.params.collectionMonth
    const collection
    db.query(`SELECT * FROM dues_collection WHERE record_id = '${duesId}' `, (err,result) => {
        if(err) {
            console.log(err)
        } else {
            // res.send(result);
