...
app.post("/newAttendance", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            let attd_type = req.body[i].attendance_for
            ...
            try{
                db.query('INSERT INTO attendance (attendance_date,attendance_month,attendance_year,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,corper_cds,attendance_type,corper_attendance,comment) VALUES(?,?,?,?,?,?,?,?,?)',
                    [date,month,year,attd_type,attd_status,c_id,c_name,c_statecode,c_batch,c_lga,c_cds], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
                        console.log(` Attedance for ${attd_type} Date:${date} month,year, Corper:${c_name} C.D.S:${c_cds}  Status: ${attd_status} recorded.`);
                    }
                })
            }catch(e){
                console.log(e)
