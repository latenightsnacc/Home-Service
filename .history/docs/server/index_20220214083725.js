...
app.post("/newAttendance", (req,res) => {
    ...
    try{
        ...
        for(var i = 0; i < lt.length; i++){
            ...
            let c_lga = req.body[i].lga;
            let c_cds = req.body[i].cds_group;
            db.query("SELECT FROM corper_attendance WHERE attendance_date = ? AND attendance_type = ? AND attendance_status = ? AND corper_id = ?",[date,attd_type,attd_status,c_id], (err, result) => {
                if(err) {
                    console.log(err)
                    ...
                    try{
                        db.query('INSERT INTO corper_attendance (attendance_date,attendance_month,attendance_year,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,corper_cds,attendance_type,attendance_status,comment) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
