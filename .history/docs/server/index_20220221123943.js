...
// POST TO DB
app.post("/createprofile", upload.single('image'), (req,res) => {
    if(!req.file) {
        console.log("No file upload");
    } else {
        console.log(req.file.filename);
        const imgSrc = 'http://localhost:3001/images'+req.file.fi
    }
})
app.get("/", (req,res) => {
...
app.post("/monthlyDues", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            db.query("SELECT * FROM monthly_dues WHERE date_recorded = ? AND payment_status = ? AND amount_paid = ? AND corper_id = ?",[collection_date,payment_status,a_paid,c_id], (err, result) => {
                if(err) {
                    console.log(err)
                    ...
                    try{
                        db.query('INSERT INTO monthly_dues (date_recorded,month,year,payment_status,amount_paid,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,corper_cds) VALUES(?,?,?,?,?,?,?,?,?,?,?)',
...
app.post("/projectDues", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            db.query("SELECT * FROM project_dues WHERE project_batch = ? AND payment_status = ? AND corper_id = ?",[project_batch,payment_status,c_id], (err, result) => {
                if(err) {
                    console.log(err)
                } else if(result.length > 0) {
                    console.log('Record exists');
...
app.post("/eventDues", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            let c_lga = req.body[i].lga;
            let c_cds = req.body[i].cds_group;
            db.query("SELECT * FROM event_dues WHERE event_title = ? AND payment_status = ? AND corper_id = ?",[event_title,payment_status,c_id], (err, result) => {
                if(err) {
                    console.log(err)
                } else if(result.length > 0) {
                    console.log('Record exists');
...
app.post("/newAttendance", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            let c_lga = req.body[i].lga;
            let c_cds = req.body[i].cds_group;
            db.query("SELECT * FROM attendance WHERE attendance_date = ? AND attendance_type = ? AND  corper_id = ?",[date,attd_type,c_id], (err, result) => {
                if(err) {
                    console.log(err)
                } else if(result.length > 0) {
                    console.log('Record exists');
                    ...
                    try{
                        db.query('INSERT INTO attendance (attendance_date,attendance_month,attendance_year,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,corper_cds,attendance_type,attendance_status,comment) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
...
app.post("/new", (req,res) => {
    ...
    try {
        ...
        db.query("SELECT * FROM  notes WHERE date = ? AND type = ? AND minutes = ? ",[date,type,minutes], (err, result) => {
            if(err) {
                console.log(err)
            } else if(result.length > 0) {
                console.log('Minutes exists');
            } else {
                db.query(`INSERT INTO notes (date,month,year, venue, topic, start_time, end_time, type, minutes) VALUES(?,?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE (minutes = ${minutes})`, [date,month,year,venue,topic,start,end,type,minutes], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
                        res.send("Minutes saved");
...
// Get Weekly Attendance from Attendance Table
app.get("/attendance/:date", (req,res) => {
    const date = req.params.date;
    
    db.query(`SELECT * FROM notes WHERE attendance_date = ? `,date, (err,result) => {
        if(err) {
            console.log(err)
        } else {
            res.send(result);
...
// Get Monthly Attendance for each month from Notes Table
app.get("/attendance/:year/:month", (req,res) => {
    const year = req.params.year;
    const month = req.params.month;
    
    db.query(`SELECT * FROM attendance WHERE attendance_year = ?  AND attendance_month = ?`,[year,month], (err,result) => {
        if(err) {
            console.log(err)
        } else {
            res.send(result);
...
app.get("/attendance", (req,res) => {
    db.query("SELECT * FROM attendance", (error, result) => {
        if(error){
            console.log(error);
        }else{
            res.send(result);
