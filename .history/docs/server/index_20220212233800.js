...
app.post("/projectDues", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            console.log([collection_for,payment_status,a_paid,c_id,c_name,c_statecode,c_batch,c_lga,c_cds]);
            try{
                db.query('INSERT INTO dues_collection (collection_for,payment_status,amount_paid,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,cds_group) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',
                    [collection_for,payment_status,a_paid,c_id,c_name,c_statecode,c_batch,c_lga,c_cds], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
                        console.log(`Corper:${c_name} Dues:${collection_for} in C.D.S:${c_cds}  Status: ${payment_status} recorded.`);
