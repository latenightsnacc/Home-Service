...
app.get("/monthlydues", (req,res) => {
    db.query("SELECT collection_date, collection_for, collection_month, payment_status, amount_paid FROM dues_collection ORDER BY date ", (err,result) => {
        if(err) {
            console.log(err)
        } else {
            res.send(result)
