...
app.get("/dues/:collectionMonth/:collectionYear/:collectionFor", (req,res) => {
    ...
    db.query(`SELECT * FROM dues_collection WHERE collection_month = '${collectionM}' AND collection_year = '${collectionY}' AND collection_for = '${collection}' `,[collectionM, collectionY], (err,result) => {
        if(err) {
            console.log(err)
        } else {
            res.send(result);
