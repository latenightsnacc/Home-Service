...
app.get("/notes/:noteId/:noteType", (req,res) => {
    const noteId = req.params.minuteId;
    const noteType = req.params.minuteType;
    const collection = req.params.collectionFor;
    db.query(`SELECT * FROM dues_collection WHERE collection_month = ? AND collection_year = ? AND collection_for = ? `,[collectionM, collectionY], (err,result) => {
        if(err) {
            console.log(err)
        } else {
            res.send(result);
