    ...
    console.log(req.body);
        try {
            ...
            for(let a in req.body) {
                ...
                corper_lga = a.lga;
                cds = a.cds_group
                db.query('INSERT INTO dues_collection (record_id	cds_group	corper_id	corper_name	corper_statecode	corper_batch	corper_lga	collection	dues	amount_paid	month	year	date
                    ) VALUES(?,?,?,?,?,?,?)', [date,venue,topic,start,end,type,minutes], (err, result) => {
    //          if(err){
    //              console.log(err)
    //          } else {
    //              res.send("Values Inserted");
