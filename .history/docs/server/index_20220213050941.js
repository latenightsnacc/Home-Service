...
app.post("/newAttendance", (req,res) => {
    ...
    try{
        for(var i = 0; i < lt.length; i++){
            ...
            let c_lga = req.body[i].lga;
            let c_cds = req.body[i].cds_group;
            console.log([date,monthattd_status,a_paid,c_id,c_name,c_statecode,c_batch,c_lga,c_cds]);
            try{
                db.query('INSERT INTO attendance (date,monthpayment_status,amount_paid,corper_id,corper_name,corper_statecode,corper_batch,corper_lga,corper_cds) VALUES(?,?,?,?,?,?,?,?,?)',
                    [date,monthpayment_status,a_paid,c_id,c_name,c_statecode,c_batch,c_lga,c_cds], (err, result) => {
                    if(err){
                        console.log(err)
                    } else {
